...
- Sessão existente, origem permitida e CSRF em toda pergunta. Limite de 20 solicitações por conta/hora, compartilhado entre sessões. Redis usa a conexão do rate limiter existente; sem Redis, o limite é por réplica. Erros e perguntas inválidas também consomem a janela depois de autenticadas. Não há retentativa automática nem contorno da cota do provedor.
- Até 2.000 caracteres na pergunta, quatro pares de histórico, 14.000 caracteres no total e 1.024 tokens de saída. Timeout de 20 segundos. Conteúdo truncado ou fora do contrato é rejeitado.
