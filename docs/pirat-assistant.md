...
- Sugestões e trechos administrativos da base são selecionados no servidor pelas permissões atuais da sessão. Operações exige `operations.read`; reprocessamento exige também `operations.manage`; equipe exige `roles.manage`. Uma declaração de administrador na conversa não concede acesso. Campos extras de permissões no corpo são rejeitados, e pares antigos sobre assuntos restritos são removidos antes de enviar o histórico ao provedor.
- Pedidos identificados de segredos, instruções internas e dados de outras contas recebem uma resposta fixa sem chamar o Gemini. Há também verificação de assuntos administrativos na resposta. Esses filtros são defesa adicional, não garantia de reconhecer toda paráfrase ou ataque: a fronteira principal é não fornecer dados privados, ferramentas nem trechos de ajuda sem permissão ao modelo. Mesmo administradores não podem obter credenciais pelo chat. Dúvidas legítimas de configuração de credenciais continuam permitidas.
- A ajuda Shopify acompanha o guia assistido: conexão, catálogo, modelo publicado e ponte no `theme.liquid` são etapas distintas. O assistente orienta o teste do carrinho sem afirmar que inspecionou a loja.
