...
- Computador: detecta a aproximação do cursor na região superior esquerda (até 200 px de largura, limitada a 35% da janela, e 100 px de altura), inclusive quando o primeiro movimento já ocorre nessa região. Se chegar antes do tempo mínimo, aguarda esse prazo enquanto o cursor continuar perto do Voltar ou fora pelo topo; voltar ao conteúdo cancela essa espera. Também detecta os 24 px superiores e a saída pelo topo, sem exigir clique ou tecla antes. Usa eventos de mouse mesmo em dispositivos com tela de toque e captura antes dos componentes da página. Não acessa a posição do cursor na barra do navegador.
