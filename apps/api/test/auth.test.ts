...
describe('autenticação administrativa', () => {
  ...
  it('cria sessão opaca em cookie HttpOnly e retorna usuário seguro', async () => {
    ...
    await app.close();
    expect(login.statusCode).toBe(200); expect(login.json<{ user: unknown }>().user).toEqual({ id: 'public-user-id', publicId: 'public-user-id', name: 'Owner', email: 'owner@example.com', accountStatus: 'APPROVED', platformAdmin: false, platformRole: null, platformPermissions: [], mfaEnabled: false });
  ...
  it('impede login enquanto a conta aguarda aprovação', async () => {
    const repository = new MemoryAuthRepository(); repository.user = { ...repository.user, accountStatus: 'PENDING' };
