...
describe('catálogo isolado por loja', () => {
  ...
  it('valida e salva personalização somente na loja autenticada', async () => {
    ...
    // Legacy drafts can contain 50 reviews plus the existing 20 elements.
    const migratedReviews = Array.from({ length: 70 }, (_, index) => ({ id: `review-${index}`, type: 'testimonial', title: `Cliente ${index}`, text: 'Avaliação fornecida.', rating: 4, slot: 0, enabled: index !== 0, region: 'main' }));
    const migrated = await app.inject({ method: 'PATCH', url: '/checkouts/checkout-a/draft', headers: authenticatedHeaders, payload: { config: { ...config, testimonials: [], customElements: migratedReviews } } });
    expect(migrated.statusCode).toBe(200);
    expect((catalog.checkouts[0]?.draftConfig as Record<string, unknown>).customElements).toHaveLength(70);
    for (const elements of [[...migratedReviews, { ...migratedReviews[0], id: 'overflow' }], migratedReviews.slice(0, 21).map(item => ({ ...item, type: 'text' }))]) {
      expect((await app.inject({ method: 'PATCH', url: '/checkouts/checkout-a/draft', headers: authenticatedHeaders, payload: { config: { ...config, customElements: elements } } })).statusCode).toBe(400);
    }
    for (const template of ['retail', 'marketplace']) {
      const saved = await app.inject({ method: 'PATCH', url: '/checkouts/checkout-a/draft', headers: authenticatedHeaders, payload: { config: { ...config, template, progressStyle: 'chevrons' } } });
      expect(saved.statusCode).toBe(200);
      expect(catalog.checkouts[0]?.draftConfig).toMatchObject({ template, progressStyle: 'chevrons' });
    }
    ...
    const offer = { ...config, exitOfferEnabled: true, exitOfferCouponCode: 'fica10', exitOfferDelaySeconds: 8, exitOfferTimedEnabled: false, exitOfferTimedSeconds: 45, exitOfferTitle: 'Espere!', exitOfferAccent: '#ab1234' };
