...
export function registerPublicCheckoutRoutes(app: FastifyInstance, environment: AppEnvironment, catalog: CatalogRepository, gateways?: PrismaGatewayRepository, shopify?: ShopifyRepository): void {
  ...
  app.post<{ Params: { sessionId: string }; Headers: { authorization?: string } }>('/public/checkout-sessions/:sessionId/payments/westpay/pix', { config: { rateLimit: { max: 5, timeWindow: '5 minutes' } } }, async (request, reply) => {
    ...
    try {
      ...
      if (provider === 'ROAS') {
        const transaction = await createRoasPix(roasCredentials, { amount: amountCents / 100, payment_method: 'pix', customer: { name: customer.name, email: customer.email, phone: brazilianPhone(customer.phone), document: { number: digits(customer.document), type: digits(customer.document).length === 14 ? 'cnpj' : 'cpf' }, external_ref: context.publicId }, items: context.items.map(item => ({ title: item.titleSnapshot, unit_price: item.unitPriceCents / 100, quantity: item.quantity, tangible: true, external_ref: item.productId })), shipping: { fee: context.shippingPriceCents / 100, address: { street: address.street, street_number: address.number, complement: address.complement || undefined, zip_code: digits(address.postalCode), neighborhood: address.neighborhood, city: address.city, state: address.state, country: 'BR' } }, pix: { expires_in_days: 1 }, external_ref: externalRef, postback_url: `${environment.API_PUBLIC_URL.replace(/\/$/, '')}/webhooks/roas`, metadata: JSON.stringify({ checkoutSession: context.publicId, platform: 'solid' }), ...(request.ip && /^\d{1,3}(?:\.\d{1,3}){3}$/.test(request.ip) ? { ip: request.ip } : {}) });
