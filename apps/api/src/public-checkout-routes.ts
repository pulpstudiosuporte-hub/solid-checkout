...
export function registerPublicCheckoutRoutes(app: FastifyInstance, environment: AppEnvironment, catalog: CatalogRepository, gateways?: PrismaGatewayRepository, shopify?: ShopifyRepository): void {
  ...
  app.post<{ Params: { sessionId: string }; Headers: { authorization?: string } }>('/public/checkout-sessions/:sessionId/payments/westpay/pix', { config: { rateLimit: { max: 5, timeWindow: '5 minutes' } } }, async (request, reply) => {
    ...
    try {
      ...
      if (provider === 'ROAS') {
        const transaction = await createRoasPix(roasCredentials, { amount: Number((amountCents / 100).toFixed(2)), payment_method: 'pix', installments: 1, customer: { name: customer.name, email: customer.email, phone: brazilianPhone(customer.phone), document: { number: digits(customer.document), type: digits(customer.document).length === 14 ? 'cnpj' : 'cpf' } }, items: context.items.map(item => ({ title: item.titleSnapshot, unit_price: Number((item.unitPriceCents / 100).toFixed(2)), quantity: item.quantity, tangible: true, external_ref: item.productId })), shipping: { fee: Number((context.shippingPriceCents / 100).toFixed(2)), address: { street: address.street, street_number: address.number, complement: address.complement || undefined, zip_code: digits(address.postalCode).replace(/^(\d{5})(\d{3})$/, '$1-$2'), neighborhood: address.neighborhood, city: address.city, state: address.state, country: 'BR' } }, pix: { expires_in_days: 1 }, postback_url: `${environment.API_PUBLIC_URL.replace(/\/$/, '')}/webhooks/roas`, metadata: JSON.stringify({ checkoutSession: context.publicId, platform: 'solid' }), ...(request.ip && /^\d{1,3}(?:\.\d{1,3}){3}$/.test(request.ip) ? { ip: request.ip } : {}) });
