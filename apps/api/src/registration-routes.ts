...
export function registerRegistrationRoutes(app: FastifyInstance, environment: AppEnvironment, database: PrismaClient): void {
  ...
  app.post<{ Body: { token?: unknown; email?: unknown; code?: unknown } }>('/auth/verify-email', { config: { rateLimit: { max: 10, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    await database.$transaction(async tx => { const current = await tx.pendingSignup.findUnique({ where: { id: pending.id } }); if (!current || !hashesMatch(current.tokenHash, expectedHash) || current.expiresAt <= new Date()) throw new Error('TOKEN_CONSUMED'); const user = await tx.user.create({ data: { email: current.email, name: current.name, passwordHash: current.passwordHash, emailVerifiedAt: new Date(), accountStatus: 'APPROVED' } }); const store = await tx.store.create({ data: { name: `${current.name.split(' ')[0]} Store`, slug: current.storeSlug } }); await tx.storeMember.create({ data: { userId: user.id, storeId: store.id, role: 'OWNER' } }); await tx.pendingSignup.delete({ where: { id: current.id } }); });
