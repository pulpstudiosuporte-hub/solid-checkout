...
export const checkoutConfig = (value: unknown): CheckoutConfigInput | null => {
  ...
  const seoDescription = optionalText(input.seoDescription, 180);
  const faviconUrl = input.faviconUrl ?? '';
  if (seoTitle === null || seoDescription === null || typeof faviconUrl !== 'string' || faviconUrl.length > 2048 || faviconUrl && !faviconUrl.startsWith('https://')) return null;
  result.seoTitle = seoTitle ?? '';
  result.seoDescription = seoDescription ?? '';
  result.faviconUrl = faviconUrl;
  const enums: Record<string, readonly string[]> = { template: ['minimal', 'conversion', 'compact', 'showcase', 'retail', 'marketplace'], font: ['Plus Jakarta Sans', 'Poppins', 'Montserrat', 'DM Sans', 'Roboto', 'Inter', 'Arial', 'Georgia'], language: ['pt-BR', 'en-US', 'es'], currency: ['BRL', 'USD', 'EUR'], buttonEffect: ['lift', 'pulse', 'shine', 'glow', 'gradient', 'press', 'none'] };
  ...
  const progressStyle = input.progressStyle ?? 'outline'; if (typeof progressStyle !== 'string' || !['outline', 'solid', 'icons', 'chevrons'].includes(progressStyle)) return null; result.progressStyle = progressStyle;
  for (const [key, fallback] of [['progressActiveColor', '#7357e9'], ['progressInactiveColor', '#ffffff'], ['progressActiveTextColor', '#ffffff'], ['progressInactiveTextColor', '#777780'], ['progressLabelColor', '#777780'], ['progressActiveLabelColor', '#17171a']] as const) { const color = hexColor(input[key], fallback); if (color === null) return null; result[key] = color; }
  ...
  for (const value of customElements) {
    ...
    const id = text(element.id, 64); const title = optionalText(element.title, 100); const body = optionalText(element.text, 500);
    const slot = integer(element.slot, 0, 4); const rating = element.rating === undefined ? 5 : integer(element.rating, 1, 5);
    const allowedTypes = ['announcement', 'banner', 'testimonial', 'timer', 'video', 'gallery', 'text', 'reviews', 'guarantee', 'faq', 'list', 'progress', 'sales', 'seal'];
    const enabled = element.enabled === undefined ? true : element.enabled; const display = element.display ?? 'fixed'; const device = element.device ?? 'all'; const align = element.align ?? 'left'; const region = element.region ?? 'main';
    ...
    const textColor = hexColor(element.textColor, '#17171a'); const backgroundColor = hexColor(element.backgroundColor, '#ffffff'); const iconColor = hexColor(element.iconColor, '#7357e9'); const iconBackgroundColor = hexColor(element.iconBackgroundColor, '#f0ebff'); const titleColor = hexColor(element.titleColor, textColor ?? '#17171a'); const bodyColor = hexColor(element.bodyColor, textColor ?? '#5f5b66'); const titleFontSize = element.titleFontSize === undefined ? 26 : integer(element.titleFontSize, 12, 64); const bodyFontSize = element.bodyFontSize === undefined ? 15 : integer(element.bodyFontSize, 10, 32); const titleWeight = element.titleWeight === undefined ? 700 : integer(element.titleWeight, 400, 900); const lineHeight = element.lineHeight === undefined ? 160 : integer(element.lineHeight, 100, 220); const imageUrl = element.imageUrl ?? ''; const mediaUrl = element.mediaUrl ?? ''; const linkUrl = element.linkUrl ?? ''; const imageFit = element.imageFit ?? 'cover'; const imageAlt = optionalText(element.imageAlt, 160);
    const hasValidContent = element.type === 'text' ? Boolean(title || body) : Boolean(title && body);
