...
export function registerAdminOperationRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, db: PrismaClient): void {
  ...
  app.get('/admin/operations', async (request, reply) => {
    if (!await adminSession(request)) return reply.code(403).send(failure(request, 'FORBIDDEN', 'Acesso administrativo necessÃ¡rio.'));
  ...
  app.post<{ Params: { jobId: string } }>('/admin/operations/:jobId/retry', async (request, reply) => {
    const session = await adminSession(request); if (!session) return reply.code(403).send(failure(request, 'FORBIDDEN', 'Acesso administrativo necessÃ¡rio.'));
    if (!mutationAllowed(request, session)) return reply.code(403).send(failure(request, 'CSRF_INVALID', 'RequisiÃ§Ã£o nÃ£o autorizada.'));
    const [kind, publicId] = request.params.jobId.split(':', 2); if (!kind || !publicId) return reply.code(400).send(failure(request, 'INVALID_JOB', 'Job invÃ¡lido.'));
    let changed = 0;
    ...
    if (!changed) return reply.code(404).send(failure(request, 'JOB_NOT_FOUND', 'Falha nÃ£o encontrada ou jÃ¡ removida.'));
