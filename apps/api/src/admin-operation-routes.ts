...
export function registerAdminOperationRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, db: PrismaClient): void {
  ...
  app.get('/admin/operations', async (request, reply) => {
    ...
    const [payments, deliveries, shopify, customerEmails, merchantEmails] = await Promise.all([
      db.paymentAttempt.findMany({ where: { OR: [{ status: 'PENDING', creationState: { in: ['CREATING', 'UNCERTAIN'] }, createdAt: { lt: new Date(Date.now() - 120_000) } }, { partialRefundAt: { not: null }, refundedAmountCents: null }, { status: 'PENDING', createdAt: { lt: new Date(Date.now() - 86400_000) } }] }, take: 100, orderBy: { createdAt: 'asc' }, select: { publicId: true, provider: true, creationState: true, partialRefundAt: true, updatedAt: true, session: { select: { publicId: true, checkout: { select: { store: { select: { name: true } } } } } } } }),
    ...
    const jobs = [
      ...payments.map(payment => ({ id: `payment:${payment.publicId}`, kind: 'Pagamento', provider: payment.provider, event: payment.partialRefundAt ? 'Conferir estorno parcial' : 'Conferir pagamento no gateway', status: 'REVIEW_REQUIRED', attempts: null, nextAttemptAt: null, error: payment.partialRefundAt ? 'O provedor ainda nao confirmou o valor estornado em centavos.' : 'Verifique a transacao original antes de liberar outra cobranca.', updatedAt: payment.updatedAt, store: payment.session.checkout.store.name, order: payment.session.publicId, retryable: false })),
    ...
    return reply.header('cache-control', 'private, no-store').send({ jobs, summary: { total: jobs.length, dead: jobs.filter(job => ['DEAD', 'REVIEW_REQUIRED'].includes(job.status)).length, retrying: jobs.filter(job => !['DEAD', 'REVIEW_REQUIRED'].includes(job.status)).length } });
  });
