...
export function registerCouponRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, catalog: CatalogRepository, database: PrismaClient): void {
  ...
  const parseInput = (body: Record<string, unknown>) => {
    ...
    const startsAt = body.startsAt ? new Date(String(body.startsAt)) : null; const expiresAt = body.expiresAt ? new Date(String(body.expiresAt)) : null;
    if (!/^[A-Z0-9_-]{3,40}$/.test(code) || !['PERCENT', 'FIXED'].includes(String(type)) || !Number.isInteger(value) || Number(value) < 1 || type === 'PERCENT' && Number(value) > 100 || !Number.isInteger(minimumSubtotalCents) || Number(minimumSubtotalCents) < 0 || maxDiscountCents !== null && (!Number.isInteger(maxDiscountCents) || Number(maxDiscountCents) < 1) || maxRedemptions !== null && (!Number.isInteger(maxRedemptions) || Number(maxRedemptions) < 1) || startsAt && Number.isNaN(startsAt.getTime()) || expiresAt && Number.isNaN(expiresAt.getTime()) || startsAt && expiresAt && startsAt >= expiresAt || typeof body.active !== 'boolean') return null;
  ...
  app.put<{ Params: { sessionId: string }; Body: { code?: string } }>('/public/checkout-sessions/:sessionId/coupon', { config: { rateLimit: { max: 10, timeWindow: '5 minutes' } } }, async (request, reply) => {
    ...
    const result = await database.$transaction(async transaction => {
      ...
      let discountCents = coupon.type === 'PERCENT' ? Math.floor(session.totalCents * coupon.value / 100) : coupon.value; if (coupon.maxDiscountCents) discountCents = Math.min(discountCents, coupon.maxDiscountCents); discountCents = Math.min(discountCents, session.totalCents - 1);
