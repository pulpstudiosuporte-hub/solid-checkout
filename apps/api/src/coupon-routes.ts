...
export function registerCouponRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, catalog: CatalogRepository, database: PrismaClient): void {
  ...
  app.get<{ Params: { sessionId: string } }>('/public/checkout-sessions/:sessionId/exit-offer', { config: { rateLimit: { max: 15, timeWindow: '1 minute' } } }, async (request, reply) => {
    reply.header('cache-control', 'private, no-store');
    const authorization = request.headers.authorization;
    if (!authorization?.startsWith('Bearer ')) return reply.code(401).send(errorBody(request, 'INVALID_SESSION', 'Sessão inválida.'));
    const now = new Date();
    const session = await database.checkoutSession.findFirst({ where: { publicId: request.params.sessionId, tokenHash: sha256(authorization.slice(7)), status: 'OPEN', expiresAt: { gt: now } }, select: { totalCents: true, couponCode: true, expiresAt: true, checkout: { select: { storeId: true, publishedConfig: true } }, paymentAttempts: { take: 1, select: { id: true } } } });
    if (!session) return reply.code(401).send(errorBody(request, 'INVALID_SESSION', 'Sessão inválida.'));
    const config = session.checkout.publishedConfig;
    if (!config || typeof config !== 'object' || Array.isArray(config) || config.exitOfferEnabled !== true || typeof config.exitOfferCouponCode !== 'string' || session.couponCode || session.paymentAttempts.length) return reply.send({ offer: null });
    const coupon = await database.coupon.findFirst({ where: { storeId: session.checkout.storeId, code: config.exitOfferCouponCode.toUpperCase(), active: true, OR: [{ startsAt: null }, { startsAt: { lte: now } }], AND: [{ OR: [{ expiresAt: null }, { expiresAt: { gt: now } }] }] }, select: { code: true, type: true, value: true, minimumSubtotalCents: true, maxDiscountCents: true, maxRedemptions: true, redemptionCount: true, expiresAt: true } });
    if (!coupon || session.totalCents < coupon.minimumSubtotalCents || (coupon.maxRedemptions !== null && coupon.redemptionCount >= coupon.maxRedemptions)) return reply.send({ offer: null });
    const couponDiscountCents = checkoutDiscounts(session.totalCents, coupon, null).discountCents;
    if (!couponDiscountCents) return reply.send({ offer: null });
    return reply.send({ offer: { code: coupon.code, type: coupon.type, value: coupon.value, couponDiscountCents, capped: coupon.type === 'PERCENT' && couponDiscountCents < Math.floor(session.totalCents * coupon.value / 10000), expiresAt: new Date(Math.min(session.expiresAt.getTime(), coupon.expiresAt?.getTime() ?? Infinity)) } });
  });
