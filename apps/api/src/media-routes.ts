...
export function registerMediaRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, catalog: CatalogRepository): void {
  ...
  const root = environment.MEDIA_STORAGE_PATH ?? '/app/uploads';
  ...
  app.post('/media/images', async (request, reply) => {
    ...
    } catch { return reply.code(400).send(error(request, 'INVALID_IMAGE', 'Não foi possível processar esta imagem.')); }
    if (output.length > 3 * 1024 * 1024) output = await sharp(output).webp({ quality: 68, effort: 4 }).toBuffer();
    const filename = `${randomUUID()}.webp`; const temporary = join(root, `.${filename}.tmp`);
    await mkdir(root, { recursive: true }); await writeFile(temporary, output); await rename(temporary, join(root, filename));
