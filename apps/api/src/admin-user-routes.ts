...
export function registerAdminUserRoutes(app: FastifyInstance, environment: AppEnvironment, auth: AuthRepository, db: PrismaClient): void {
  ...
  const adminSession = async (request: FastifyRequest, permission: 'users.read' | 'users.manage' | 'billing.manage' = 'users.read'): Promise<SessionUser | null> => {
  ...
  app.get<{ Querystring: { status?: string; page?: string; search?: string } }>('/admin/users', async (request, reply) => {
    ...
    const pageSize = 30;
    const search = (request.query.search ?? '').trim().slice(0, 120);
    const where = { accountStatus: status, ...(search ? { OR: [{ name: { contains: search, mode: 'insensitive' as const } }, { email: { contains: search, mode: 'insensitive' as const } }] } : {}) };
    const [total, users] = await Promise.all([
      db.user.count({ where }),
      db.user.findMany({
        where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * pageSize, take: pageSize,
        select: { publicId: true, name: true, email: true, accountStatus: true, platformAdmin: true, platformRole: { select: { publicId: true, name: true } }, disabledAt: true, emailVerifiedAt: true, createdAt: true, billingSubscription: true, memberships: { select: { role: true, store: { select: { name: true, publicId: true } } } } }
      })
    ]);
  ...
  app.patch<{ Params: { publicId: string }; Body: { plan?: unknown; feeBasisPoints?: unknown; monthlyWaived?: unknown; expiresAt?: unknown; reason?: unknown } }>('/admin/users/:publicId/billing-override', async (request, reply) => {
    const session = await adminSession(request, 'billing.manage');
    if (!session) return reply.code(403).send(failure(request, 'FORBIDDEN', 'Acesso administrativo necessário.'));
    if (!mutationAllowed(request, session)) return reply.code(403).send(failure(request, 'CSRF_INVALID', 'Requisição não autorizada.'));
    const target = await db.user.findUnique({ where: { publicId: request.params.publicId }, select: { id: true, publicId: true, platformAdmin: true, platformRoleId: true, billingSubscription: true } });
    if (!target) return reply.code(404).send(failure(request, 'USER_NOT_FOUND', 'Usuário não encontrado.'));
    if (target.platformAdmin || target.platformRoleId) return reply.code(409).send(failure(request, 'ADMIN_OVERRIDE_FORBIDDEN', 'Use este benefício apenas em contas de clientes.'));
  ...
  app.post<{ Params: { publicId: string } }>('/admin/users/:publicId/approve', async (request, reply) => {
    const session = await adminSession(request, 'users.manage');
    if (!session) return reply.code(403).send(failure(request, 'FORBIDDEN', 'Acesso administrativo necessário.'));
    if (!mutationAllowed(request, session)) return reply.code(403).send(failure(request, 'CSRF_INVALID', 'Requisição não autorizada.'));
    const target = await db.user.findUnique({ where: { publicId: request.params.publicId }, select: { id: true, publicId: true, platformAdmin: true, platformRoleId: true } });
    if (!target) return reply.code(404).send(failure(request, 'USER_NOT_FOUND', 'Usuário não encontrado.'));
    if (target.platformAdmin || target.platformRoleId) return reply.code(409).send(failure(request, 'STAFF_PROTECTED', 'Gerencie a equipe pela tela de perfis.'));
  ...
  app.post<{ Params: { publicId: string } }>('/admin/users/:publicId/block', async (request, reply) => {
    ...
    const target = await db.user.findUnique({ where: { publicId: request.params.publicId }, select: { id: true, publicId: true, platformAdmin: true, platformRoleId: true } });
    if (!target) return reply.code(404).send(failure(request, 'USER_NOT_FOUND', 'Usuário não encontrado.'));
    if (target.platformAdmin || target.platformRoleId) return reply.code(409).send(failure(request, 'ADMIN_BLOCK_FORBIDDEN', 'Outro administrador da plataforma não pode ser bloqueado por esta tela.'));
    const now = new Date();
    await db.$transaction([
