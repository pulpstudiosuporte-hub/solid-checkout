...
export async function syncMetaEvent(environment: AppEnvironment, repository: PrismaGatewayRepository, checkoutSessionId: string, eventName: MetaEventName, log: FastifyBaseLogger): Promise<void> {
  ...
  const credentials = await repository.credentials(context.checkout.storeId, 'META'); if (!credentials) return;
  try {
    ...
    const total = context.totalCents - context.discountCents + context.shippingPriceCents; const items = context.items.length ? context.items : [{ productId: context.checkout.product.publicId, titleSnapshot: context.checkout.product.checkoutTitle, unitPriceCents: context.totalCents, quantity: 1, product: { publicId: context.checkout.product.publicId } }];
  ...
  } catch (error) { log.warn({ err: error, checkoutSessionId, metaEvent: eventName }, 'meta_capi_sync_failed'); try { await repository.recordIntegrationEvent(context.checkout.storeId, 'META', eventName, false, { checkoutSessionId: context.publicId }); } catch {} }
}
