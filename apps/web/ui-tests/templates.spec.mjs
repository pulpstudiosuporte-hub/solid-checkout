...
for (const template of ['retail', 'marketplace']) {
  for (const width of [1440, 390]) test(`${template} ${width}: identificação, entrega e Pix`, async ({ page }) => {
    const errors = []; page.on('pageerror', error => errors.push(error.message));
    await page.setViewportSize({ width, height: 950 });
    await page.goto(`/ui-tests/templates-review.html?template=${template}`);
