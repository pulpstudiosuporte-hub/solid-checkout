...
export default function Login({ onSubmit, onMfaSubmit, onRegister, onVerify }) {
  ...
  const [mfaCode, setMfaCode] = useState(''); const [mfaChallenge, setMfaChallenge] = useState(null);
  ...
  const changeMode = next => { setMode(next); setError(''); setNotice(''); setMfaChallenge(null); setMfaCode(''); window.history.replaceState({}, '', next === 'register' ? '/#/cadastro' : '/#/login'); };
  return <main className="login-page">
    ...
    <section className="login-form-panel"><form className="login-card" onSubmit={submit} noValidate>
      ...
      <p className="eyebrow">{mode === 'register' ? 'COMECE AGORA' : mode === 'verify' ? 'VALIDAÇÃO SEGURA' : mode === 'mfa' ? 'SEGUNDO FATOR' : 'PAINEL ADMINISTRATIVO'}</p>
      <h2>{mode === 'register' ? 'Crie sua conta' : mode === 'verify' ? 'Confirmando seu e-mail' : mode === 'mfa' ? 'Confirme que é você' : 'Bem-vindo de volta'}</h2>
      <p className="login-subtitle">{mode === 'register' ? 'Sua primeira loja será criada após confirmar o e-mail.' : mode === 'verify' ? 'Aguarde enquanto validamos seu acesso.' : mode === 'mfa' ? 'Digite o código do aplicativo autenticador ou um código de recuperação.' : 'Entre com os dados cadastrados para acessar sua loja.'}</p>
      ...
      {mode !== 'verify' && <button className="login-submit" type="submit" disabled={loading || (mode === 'mfa' ? mfaCode.trim().length < 6 : !email || password.length < (mode === 'register' ? 14 : 1) || mode === 'register' && (!name || !terms))}>{loading ? <><LoaderCircle className="spin" size={19}/> Aguarde...</> : <>{mode === 'register' ? 'Criar minha conta' : mode === 'mfa' ? 'Validar e entrar' : 'Entrar no painel'} <ArrowRight size={19}/></>}</button>}
      {mode === 'verify' && loading && <div className="login-help"><LoaderCircle className="spin" size={20}/> Validando...</div>}
      {mode !== 'mfa' && <p className="login-help">{mode === 'register' ? <>Já possui uma conta? <button type="button" onClick={() => changeMode('login')}>Entrar</button></> : <>Ainda não possui conta? <button type="button" onClick={() => changeMode('register')}>Criar conta</button></>}</p>}
    </form></section>
  </main>;
}
