...
export default function Login({ onSubmit, onMfaSubmit, onRegister, onVerify, onForgot, onReset }) {
  ...
  const automaticVerification = resetRoute ? null : urlToken ? { token: urlToken } : urlVerificationEmail && urlVerificationCode ? { email: urlVerificationEmail, code: urlVerificationCode } : null;
  ...
  React.useEffect(() => { if (mode !== 'verify' || !automaticVerification) return; setLoading(true); onVerify(automaticVerification).then(() => { setEmail(urlVerificationEmail || ''); setNotice('E-mail confirmado. Sua conta e sua loja já estão liberadas para entrar.'); setMode('login'); window.history.replaceState({}, '', '/#/login'); }).catch(requestError => setError(requestError?.message || 'O código é inválido ou expirou.')).finally(() => setLoading(false)); }, []);
