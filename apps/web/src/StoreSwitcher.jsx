...
export default function StoreSwitcher({ stores, onSelect, onCreate, onArchive, busy, support = false }) {
  ...
  return <div className={`store-switcher ${hasStores ? '' : 'is-empty'}`}>
    ...
    {open && <><button className="store-menu-backdrop" onClick={() => setOpen(false)} aria-label="Fechar seletor de lojas"/><div className="store-menu" role="listbox" aria-label="Selecionar loja">
      ...
      <div className="store-list">{stores.map(store => <div key={store.publicId} className="store-list-row"><button role="option" aria-selected={store.active} disabled={busy || store.active} onClick={async () => { await onSelect(store.publicId); setOpen(false); }}><span className="store-avatar">{initials(store.name)}</span><span><b>{store.name}</b><small>{store.slug}</small></span>{store.active ? <Check size={17}/> : busy ? <LoaderCircle className="spin" size={16}/> : null}</button>{!support && stores.length > 1 && store.role === 'OWNER' && <button className="store-archive" disabled={busy} onClick={async () => { if (!window.confirm(`Arquivar a loja “${store.name}”? Pedidos e histórico serão preservados.`)) return; await onArchive(store.publicId); setOpen(false); }} aria-label={`Arquivar ${store.name}`}>Arquivar</button>}</div>)}</div>
      {!support && <button className="create-store-trigger" onClick={() => setCreating(true)}><span><Plus size={17}/></span> Criar nova loja</button>}
    </div></>}
