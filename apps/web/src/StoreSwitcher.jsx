...
export default function StoreSwitcher({ stores, onSelect, onCreate, onArchive, busy }) {
  ...
  return <div className={`store-switcher ${hasStores ? '' : 'is-empty'}`}>
    <button ref={triggerRef} className="store-switch" onClick={() => hasStores ? setOpen(value => !value) : setCreating(true)} aria-expanded={hasStores ? open : creating} aria-haspopup={hasStores ? 'listbox' : 'dialog'}>
      <span className="store-avatar">{hasStores ? initials(activeStore?.name) : <Store size={18}/>}</span>
      <span><b>{activeStore?.name || 'Crie sua primeira loja'}</b><small>{activeStore?.slug || 'Comece sua operação na SOLID'}</small></span>
    ...
    {open && <><button className="store-menu-backdrop" onClick={() => setOpen(false)} aria-label="Fechar seletor de lojas"/><div className="store-menu" role="listbox" aria-label="Selecionar loja">
      ...
      <div className="store-list">{stores.map(store => <div key={store.publicId} className="store-list-row"><button role="option" aria-selected={store.active} disabled={busy || store.active} onClick={async () => { await onSelect(store.publicId); setOpen(false); }}><span className="store-avatar">{initials(store.name)}</span><span><b>{store.name}</b><small>{store.slug}</small></span>{store.active ? <Check size={17}/> : busy ? <LoaderCircle className="spin" size={16}/> : null}</button>{stores.length > 1 && store.role === 'OWNER' && <button className="store-archive" disabled={busy} onClick={async () => { if (!window.confirm(`Arquivar a loja “${store.name}”? Pedidos e histórico serão preservados.`)) return; await onArchive(store.publicId); setOpen(false); }} aria-label={`Arquivar ${store.name}`}>Arquivar</button>}</div>)}</div>
      <button className="create-store-trigger" onClick={() => setCreating(true)}><span><Plus size={17}/></span> Criar nova loja</button>
    </div></>}
    {creating && <div className="modal-overlay" role="presentation" onMouseDown={event => event.target === event.currentTarget && closeCreate()}><section ref={modalRef} className="store-modal" role="dialog" aria-modal="true" aria-labelledby="create-store-title"><button className="modal-close" onClick={closeCreate} aria-label="Fechar"><X size={19}/></button><div className="store-modal-head"><span><Store size={21}/></span><div><h2 id="create-store-title">{hasStores ? 'Criar nova loja' : 'Crie sua primeira loja'}</h2><p>{hasStores ? 'Escolha um nome para identificar sua operação.' : 'Você poderá ajustar os demais dados depois.'}</p></div></div><form onSubmit={submit}><label htmlFor="store-name">Nome da loja</label><input id="store-name" value={name} onChange={event => setName(event.target.value)} placeholder="Ex.: Minha Loja Incrível" maxLength={120} autoFocus disabled={busy}/><small>Use entre 3 e 120 caracteres.</small>{error && <p className="store-form-error" role="alert">{error}</p>}<div className="store-modal-actions"><button type="button" className="secondary" onClick={closeCreate}>Cancelar</button><button type="submit" className="primary" disabled={busy || name.trim().length < 3}>{busy ? <><LoaderCircle className="spin" size={17}/> Criando...</> : hasStores ? 'Criar loja' : 'Criar minha loja'}</button></div></form></section></div>}
  </div>;
}
