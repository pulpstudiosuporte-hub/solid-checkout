...
function AreaChart({ series, mode = 'revenue' }) {
  ...
  return <>
    <div className="analytics-area-chart"><div className="analytics-chart-scale"><span>{mode === 'revenue' ? money.format(max / 100) : number.format(max)}</span><span>{mode === 'revenue' ? 'R$ 0' : '0'}</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={mode === 'revenue' ? 'Receita no período. Valores diários disponíveis na tabela abaixo.' : 'Pedidos no período'}><defs><linearGradient id={gradient} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#7357e9" stopOpacity=".3"/><stop offset="1" stopColor="#7357e9" stopOpacity="0"/></linearGradient></defs>{[20,44,68,92].map(y => <line key={y} x1="0" x2="100" y1={y} y2={y}/>)}<path d={area} fill={`url(#${gradient})`}/><path d={line} className="analytics-chart-line"/>{points.length === 1 && <circle cx={points[0].x} cy={points[0].y} r="1.5" fill="#7357e9"/>}</svg><div className="analytics-chart-dates">{labels.map(index => <span key={index}>{series[index].date.slice(8,10)}/{series[index].date.slice(5,7)}</span>)}</div></div>
