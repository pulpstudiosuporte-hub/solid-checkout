...
export function LiveAnalytics() {
  ...
  const simulate = () => {
    ...
    setSelected(null); setRevision(value => value + 1); setNotice('Cenário atualizado: + R$ 149 na receita.');
  };
  return <section className="site-section site-container analytics-section" id="em-movimento" ref={root}>
    <div className="section-heading"><span className="site-eyebrow"><Activity size={14}/> VEJA A EXPERIÊNCIA GANHAR VIDA</span><h2>Sua operação.<br/><span>Em movimento.</span></h2><p>Altere o período, acompanhe o gráfico e explore cenários de venda.<br/>Aqui, cada interação conta uma parte da história.</p></div>
    <div className="motion-dashboard" data-revision={revision}>
      <div className="dashboard-toolbar"><div className="dashboard-title"><span className="dashboard-symbol"><BarChart3 size={22}/></span><div><strong>Visão da operação</strong><span><i className="live-dot"/> Explore seus indicadores</span></div></div><div className="chart-period" role="group" aria-label="Período do gráfico">{[7, 30].map(days => <button type="button" key={days} aria-pressed={period === days} onClick={() => changePeriod(days)}>{days} dias</button>)}</div></div>
      <div className="dashboard-metrics"><div><span>Receita no cenário <TrendingUp size={16}/></span><strong><AnimatedNumber value={orders * 149} money/></strong><small>Total do período selecionado</small></div><div><span>Pedidos no cenário <ShoppingBagIcon/></span><strong><AnimatedNumber value={orders}/></strong><small>Explore os pontos para ver o detalhe</small></div><div className="metric-action"><span className="simulation-orbit" aria-hidden="true"><Sparkles size={24}/></span><button className="site-button" type="button" onClick={simulate}><Plus size={16}/> Adicionar ao cenário</button></div></div>
      <div className="dashboard-body"><div className="main-chart"><div className="chart-heading"><div role="group" aria-label="Métrica do gráfico" className="chart-metrics">{[['revenue', 'Receita'], ['orders', 'Pedidos']].map(([value, title]) => <button type="button" key={value} aria-pressed={metric === value} onClick={() => setMetric(value)}>{title}</button>)}</div><span className="chart-hint"><MousePointer2 size={13}/> Toque ou passe o mouse</span></div>
        ...
        <div className="chart-bottom"><span><i/>{metric === 'revenue' ? 'Receita' : 'Pedidos'} no período selecionado</span><small>{!running ? 'Animação pausada' : inspecting || focused || tableOpen ? 'Pausado para explorar' : 'Atualiza a cada 3 segundos'}</small></div>
      ...
      <div className="dashboard-footnote"><span role="status">{notice || 'Altere o período e explore os detalhes de cada ponto.'}</span><span><Check size={13}/> Explore cenários de venda</span></div>
      <details className="chart-data" onToggle={event => setTableOpen(event.currentTarget.open)}><summary>Ver os dados do gráfico em tabela</summary><div><table><caption>Cenário interativo · {period} dias</caption><thead><tr><th scope="col">Período</th><th scope="col">Pedidos</th><th scope="col">Receita</th></tr></thead><tbody>{samples.map((value, index) => <tr key={index}><th scope="row">{label(index)}</th><td>{value}</td><td>{currency(value * 149)}</td></tr>)}</tbody></table></div></details>
    </div>
  </section>;
}
