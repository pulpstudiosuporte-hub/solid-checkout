...
export default function NewsRoadmapPage({ csrfToken }) {
  ...
  const submit = async input => { setBusy(true); setMessage(''); try { const result = await createProductFeedback(input, csrfToken); setFeedback(current => ({ ...current, items: [result.feedback, ...current.items] })); setDialog(null); setTab('roadmap'); setMessage(input.type === 'BUG' ? 'Problema enviado para a equipe.' : 'Sugestão publicada no roadmap.'); } catch (error) { setMessage(error.message); } finally { setBusy(false); } };
