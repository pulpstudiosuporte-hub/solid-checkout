...
function ImageField({ value, onChange, csrfToken, label = 'Imagem' }) {
  ...
  const upload = async event => {
    ...
    try { const result = await uploadPlatformImage(file, csrfToken); onChange(result.imageUrl); }
...
export default function AdminContentPage({ csrfToken }) {
  ...
  if (state.loading && !state.data) content = <div className="admin-content-loading"><LoaderCircle className="spin"/> Carregando central...</div>;
    ...
    <form className="card admin-content-form" onSubmit={saveAsset}><h2>Imagem da integração ou gateway</h2>
      <label>Item<select value={asset.integrationKey} onChange={event => { const integrationKey = event.target.value; const current = state.data?.integrationAssets?.find(item => item.integrationKey === integrationKey); setAsset({ integrationKey, imageUrl: current?.imageUrl || '', altText: current?.altText || '' }); }}><optgroup label="Integrações">{integrations.map(([id,name]) => <option key={id} value={id}>{name}</option>)}</optgroup><optgroup label="Gateways">{gatewayAssetOptions.map(([id,name]) => <option key={id} value={id}>{name}</option>)}</optgroup></select></label>
    ...
    <section className="card admin-assets-list"><h2>Imagens configuradas</h2>{assetOptions.map(([id,name], index) => { const current = state.data?.integrationAssets?.find(item => item.integrationKey === id); return <React.Fragment key={id}>{index === 0 && <h3 className="admin-assets-group-title">Integrações</h3>}{index === integrations.length && <h3 className="admin-assets-group-title">Gateways</h3>}<button type="button" onClick={() => setAsset({ integrationKey: id, imageUrl: current?.imageUrl || '', altText: current?.altText || '' })}>{current ? <img src={current.imageUrl} alt={current.altText}/> : <span><Image size={18}/></span>}<b>{name}</b><small>{current ? 'Configurada' : 'Usando ícone padrão'}</small></button></React.Fragment>; })}</section>
  </div>;
