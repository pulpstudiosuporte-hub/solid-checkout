...
export default function DashboardPage({ setPage, storeKey }) {
  ...
  useEffect(() => {
    ...
    getDashboard('today', controller.signal, storeKey ? { store: storeKey } : {}).then(data => { if (!controller.signal.aborted) setState({ loading: false, data, error: '', storeKey }); }).catch(error => {
      if (!controller.signal.aborted) setState(current => ({ ...current, loading: false, error: error.message, storeKey }));
    });
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    getDashboard(geoPeriod, controller.signal, storeKey ? { store: storeKey } : {}).then(data => { if (!controller.signal.aborted) setGeoState({ data, error: '', storeKey, period: geoPeriod }); }).catch(error => {
      if (!controller.signal.aborted) setGeoState({ data: null, error: error.message, storeKey, period: geoPeriod });
    });
    return () => controller.abort();
  }, [geoPeriod, storeKey]);
  ...
  return <main className="page home-overview">
    <header className="home-greeting"><div className="home-greeting-copy"><p className="eyebrow"><Compass size={16} aria-hidden="true"/> SEU PAINEL DE BORDO</p><h1>{greeting}, {firstName}!</h1><p>Sua loja no rumo certo. Cada resultado, à vista.</p></div><div className="home-greeting-art" aria-hidden="true"><Compass/><img src="/brand/pirat-mascot.png" alt="" width="108" height="108"/></div><div className="home-greeting-actions"><span className="home-today">{new Intl.DateTimeFormat('pt-BR', { day: 'numeric', month: 'long' }).format(new Date())}</span><button className="secondary" onClick={() => setPage('Análises')}><TrendingUp size={16}/> Ver análises completas</button></div></header>
    ...
    <section className="home-main-grid">
      ...
      <aside className="card home-news"><div className="home-news-cover"><span>NOVIDADES Pirat</span><b>{newsState.items[0]?.title || 'Acompanhe a evolução da plataforma'}</b></div>{newsState.loading ? <div className="home-news-state"><LoaderCircle className="spin"/> Carregando novidades...</div> : newsState.error ? <div className="home-news-state error"><AlertCircle size={17}/> Novidades indisponíveis agora</div> : newsItems.length ? newsItems.map(([id,title,time]) => <button key={id} onClick={() => setPage('Novidades')}><span>{title}</span><small>{time}</small><ArrowRight size={15}/></button>) : <div className="home-news-state"><Sparkles size={17}/> Nenhuma novidade publicada</div>}<button className="home-news-cta" onClick={() => setPage('Novidades')}>Ver novidades e roadmap <ArrowRight size={16}/></button></aside>
    </section>
    <section className="home-tools"><h2>Seu próximo movimento</h2><p>As ferramentas certas para levar sua loja mais longe.</p><div><button onClick={() => setPage('Checkouts')}><i className="home-tool-icon"><LayoutTemplate size={22} aria-hidden="true"/></i><b>Personalize seu checkout</b><span>Edite layout, conteúdo e elementos de conversão.</span><ArrowRight size={17}/></button><button onClick={() => setPage('Order bumps')}><i className="home-tool-icon"><Sparkles size={22} aria-hidden="true"/></i><b>Aumente o ticket médio</b><span>Crie ofertas complementares no checkout.</span><ArrowRight size={17}/></button><button onClick={() => setPage('Carrinhos')}><i className="home-tool-icon"><RotateCcw size={22} aria-hidden="true"/></i><b>Recupere vendas</b><span>Acompanhe oportunidades que não foram concluídas.</span><ArrowRight size={17}/></button></div></section>
  </main>;
}
