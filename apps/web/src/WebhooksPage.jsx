...
export default function WebhooksPage({ storeKey, csrfToken }) {
  ...
  const test = async item => { setTesting(item.publicId); setMenu(null); try { await testStoreWebhook(item.publicId, item.events[0] || 'order.created', csrfToken); setNotice({ ok: true, text: 'Teste agendado para envio. O resultado aparecerá no cartão.' }); load(); window.setTimeout(load, 18_000); } catch (error) { setNotice({ ok: false, text: error.message }); } finally { setTesting(''); } };
