...
function WebhooksWorkspace({ storeKey, csrfToken }) {
  ...
  return <main className="page webhooks-page"><div className="webhook-breadcrumb">Sistema <span>›</span> Webhooks</div><section className="page-title"><div><h1>Webhooks</h1><p>Receba notificações em tempo real sobre eventos da sua loja.</p></div>{data.writable && <button className="primary" onClick={() => setEditing({})}><Plus/> Novo webhook</button>}</section>
    ...
    {data.loading ? <div className="webhook-state"><LoaderCircle className="spin"/> Carregando webhooks...</div> : data.error ? <div className="webhook-state error">{data.error}<button className="secondary" onClick={load}>Tentar novamente</button></div> : !data.items.length ? <div className="card webhook-empty"><Webhook/><h2>Nenhum webhook configurado</h2><p>Conecte seu ERP, automação ou sistema interno aos eventos da SOLID.</p>{data.writable && <button className="primary" onClick={() => setEditing({})}><Plus/> Criar primeiro webhook</button>}</div> : <div className="webhook-grid">{data.items.map(item => <article className="card webhook-card" key={item.publicId}><header><h2>{item.name}</h2><span className={item.active ? 'active' : 'inactive'}>{item.active ? 'Ativo' : 'Inativo'}</span>{data.writable && <button aria-label={`Ações de ${item.name}`} aria-haspopup="menu" aria-expanded={menu === item.publicId} onClick={event => { event.stopPropagation(); setMenu(menu === item.publicId ? null : item.publicId); }}><EllipsisVertical/></button>}{data.writable && menu === item.publicId && <div className="webhook-menu" role="menu"><button role="menuitem" onClick={() => setEditing(item)}>Editar</button><button role="menuitem" onClick={() => test(item)} disabled={!item.active || testing === item.publicId}>{testing === item.publicId ? 'Enviando...' : 'Enviar teste'}</button><button role="menuitem" className="danger" onClick={() => setDeleting(item)}><Trash2/> Excluir</button></div>}</header><a href={item.url} target="_blank" rel="noreferrer"><Link2/>{item.url}<ExternalLink/></a>{item.description && <p>{item.description}</p>}<div className="webhook-tags">{item.events.map(event => <span key={event}>{eventOptions.find(option => option[0] === event)?.[1] || event}</span>)}</div><footer><span>{item._count.deliveries} envios</span>{item.deliveries[0] && <span className={item.deliveries[0].success ? 'ok' : ['PENDING', 'PROCESSING'].includes(item.deliveries[0].status) ? '' : 'fail'}>{item.deliveries[0].success ? 'Último envio entregue' : ['PENDING', 'PROCESSING'].includes(item.deliveries[0].status) ? 'Envio agendado' : 'Último envio falhou'}</span>}{data.writable && <button onClick={() => test(item)} disabled={!item.active || testing === item.publicId} aria-label="Enviar teste"><Send/></button>}</footer></article>)}</div>}
