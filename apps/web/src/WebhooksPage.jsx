...
function Dialog({ initial, onClose, onSaved, csrfToken }) {
  const ref = useRef(null); useDialogFocus(ref, () => { if (!busy) onClose(); });
  const [form, setForm] = useState(initial ? { ...initial, secret: '' } : blank);
  const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [revealed, setRevealed] = useState(''); const [copyMessage, setCopyMessage] = useState('');
  const copySecret = async () => { try { await navigator.clipboard.writeText(revealed); setCopyMessage('Chave copiada.'); } catch { setCopyMessage('Não foi possível copiar. Selecione a chave e copie manualmente.'); } };
  ...
  return <div className="webhook-overlay" onMouseDown={event => event.target === event.currentTarget && !busy && onClose()}><section ref={ref} className="webhook-dialog" role="dialog" aria-modal="true" aria-labelledby="webhook-dialog-title">
    <header><div><h2 id="webhook-dialog-title">{initial ? 'Editar webhook' : 'Novo webhook'}</h2><p>Receba notificações em tempo real dos eventos da sua loja.</p></div><label className="webhook-active"><input type="checkbox" disabled={busy || Boolean(revealed)} checked={form.active} onChange={event => setForm({ ...form, active: event.target.checked })}/><i/><span>Ativo</span></label><button type="button" disabled={busy} onClick={onClose} aria-label="Fechar"><X/></button></header>
    {revealed ? <div className="webhook-secret"><Check/><h3>Webhook criado</h3><p>Copie a chave secreta agora. Ela não será exibida novamente.</p><textarea aria-label="Chave secreta do webhook" readOnly value={revealed} onFocus={event => event.target.select()}/><button className="secondary" onClick={copySecret}><Copy size={16}/> Copiar chave</button>{copyMessage && <p role="status">{copyMessage}</p>}<button className="primary" onClick={onSaved}>Concluir</button></div> : <form onSubmit={submit}>
      ...
      {error && <p className="webhook-error" role="alert">{error}</p>}<footer><button type="button" className="secondary" disabled={busy} onClick={onClose}>Cancelar</button><button className="primary" disabled={busy || !form.name.trim() || !form.url || !form.events.length}>{busy ? <LoaderCircle className="spin"/> : 'Salvar webhook'}</button></footer>
    </form>}
  </section></div>;
}
...
export default function WebhooksPage(props) {
  return <WebhooksWorkspace key={props.storeKey} {...props}/>;
}

function WebhooksWorkspace({ storeKey, csrfToken }) {
  const support = useSupportAccess();
  const canEditInSupport = !support || support.mode === 'FULL_ACCESS';
  ...
  useEffect(() => {
    ...
    getStoreWebhooks(controller.signal).then(value => {
      if (!controller.signal.aborted) setData({ loading: false, error: '', ...value });
    }).catch(error => {
      if (!controller.signal.aborted && error.name !== 'AbortError') setData(value => ({ ...value, loading: false, error: error.message }));
    });
    return () => controller.abort();
  }, [storeKey, revision]);
  useEffect(() => {
    if (!data.items.some(item => item.deliveries?.some(delivery => ['PENDING', 'PROCESSING'].includes(delivery.status)))) return;
  ...
  const test = async item => { if (!data.writable || !item.active) return; setTesting(item.publicId); setMenu(null); try { await testStoreWebhook(item.publicId, item.events[0] || 'order.created', csrfToken); setNotice({ ok: true, text: 'Teste agendado para envio. O resultado aparecerá no cartão.' }); load(); } catch (error) { setNotice({ ok: false, text: error.message }); } finally { setTesting(''); } };
  return <main className="page webhooks-page"><div className="webhook-breadcrumb">Sistema <span>›</span> Webhooks</div><section className="page-title"><div><h1>Webhooks</h1><p>Receba notificações em tempo real sobre eventos da sua loja.</p></div>{canEditInSupport && data.writable && <button className="primary" onClick={() => setEditing({})}><Plus/> Novo webhook</button>}</section>
    ...
    {data.loading ? <div className="webhook-state"><LoaderCircle className="spin"/> Carregando webhooks...</div> : data.error ? <div className="webhook-state error">{data.error}<button className="secondary" onClick={load}>Tentar novamente</button></div> : !data.items.length ? <div className="card webhook-empty"><Webhook/><h2>Nenhum webhook configurado</h2><p>Conecte seu ERP, automação ou sistema interno aos eventos da SOLID.</p>{canEditInSupport && data.writable && <button className="primary" onClick={() => setEditing({})}><Plus/> Criar primeiro webhook</button>}</div> : <div className="webhook-grid">{data.items.map(item => <article className="card webhook-card" key={item.publicId}><header><h2>{item.name}</h2><span className={item.active ? 'active' : 'inactive'}>{item.active ? 'Ativo' : 'Inativo'}</span>{canEditInSupport && data.writable && <button aria-label={`Ações de ${item.name}`} aria-haspopup="menu" aria-expanded={menu === item.publicId} onClick={event => { event.stopPropagation(); setMenu(menu === item.publicId ? null : item.publicId); }}><EllipsisVertical/></button>}{canEditInSupport && data.writable && menu === item.publicId && <div className="webhook-menu" role="menu"><button role="menuitem" onClick={() => setEditing(item)}>Editar</button><button role="menuitem" onClick={() => test(item)} disabled={!item.active || testing === item.publicId}>{testing === item.publicId ? 'Enviando...' : 'Enviar teste'}</button><button role="menuitem" className="danger" onClick={() => setDeleting(item)}><Trash2/> Excluir</button></div>}</header><a href={item.url} target="_blank" rel="noreferrer"><Link2/>{item.url}<ExternalLink/></a>{item.description && <p>{item.description}</p>}<div className="webhook-tags">{item.events.map(event => <span key={event}>{eventOptions.find(option => option[0] === event)?.[1] || event}</span>)}</div><footer><span>{item._count.deliveries} envios</span>{item.deliveries[0] && <span className={item.deliveries[0].success ? 'ok' : ['PENDING', 'PROCESSING'].includes(item.deliveries[0].status) ? '' : 'fail'}>{item.deliveries[0].success ? 'Último envio entregue' : ['PENDING', 'PROCESSING'].includes(item.deliveries[0].status) ? 'Envio agendado' : 'Último envio falhou'}</span>}{canEditInSupport && data.writable && <button onClick={() => test(item)} disabled={!item.active || testing === item.publicId} aria-label="Enviar teste"><Send/></button>}</footer></article>)}</div>}
