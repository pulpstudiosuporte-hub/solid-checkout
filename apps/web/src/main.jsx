...
function App(){
  ...
  const publicRoute = Boolean(publicMatch || publicSessionMatch);
  ...
  useEffect(()=>{let active=true; getApiHealth().then(()=>active&&setApiStatus('online')).catch(()=>active&&setApiStatus('offline')); if(publicRoute){setAuth({status:'anonymous',user:null,csrfToken:null});return()=>{active=false}} getSession().then(result=>{if(!active)return;bindTabToUser(result.user.publicId || result.user.id);setAuth({status:'authenticated',user:result.user,csrfToken:result.csrfToken})}).catch(error=>{if(!active)return;if(error?.code==='SESSION_CONTEXT_CHANGED')setSessionConflict(true);else setAuth({status:'anonymous',user:null,csrfToken:null})}); return()=>{active=false}},[publicRoute]);
