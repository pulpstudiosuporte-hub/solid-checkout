...
export default function ExitOfferSettings({ config, update }) {
  ...
  return <div className="exit-offer-settings"><h3>Oferta de saída</h3><p className="panel-help">Ofereça um cupom quando o comprador demonstrar intenção de sair. Aparece uma vez por sessão, antes de gerar o pagamento.</p>
    ...
    <label>Tempo mínimo na página (segundos)<input type="number" min="5" max="120" value={c.exitOfferDelaySeconds} onChange={event => update('exitOfferDelaySeconds', Number(event.target.value))}/></label>
    ...
    <p className="panel-help">No computador, detecta a saída do cursor pelo topo. Não bloqueia o botão Voltar nem o fechamento da aba. O contador não reinicia ao recarregar.</p>
