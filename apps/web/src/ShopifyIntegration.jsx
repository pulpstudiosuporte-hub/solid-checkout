...
function ShopifyIntegrationDetails({ csrfToken, storeKey, onBack }) {
  ...
  return <main className="page integrations-page integration-detail-page"><button className="integration-back" type="button" onClick={onBack}><ArrowLeft size={16}/> Voltar para integrações</button><section className="page-title"><div><p className="eyebrow">INTEGRAÇÕES</p><h1>Shopify</h1><p>Importe o catálogo completo da loja ativa com autorização segura.</p></div><span className={`connection-pill ${status.connected ? 'connected' : ''} ${status.reconnectRequired ? 'reconnect' : ''}`}>{badge}</span></section>
    ...
    </div><aside className="card integration-security"><span><ShieldCheck size={23}/></span><h2>Credenciais protegidas</h2><p>A SOLID usa o app criado por você somente para esta loja.</p><ul><li>Client secret criptografado no banco</li><li>Token temporário renovado automaticamente</li><li>Permissões mínimas: produtos e pedidos</li><li>Senha da Shopify nunca solicitada</li></ul><p className="integration-scope-note"><strong>Importante:</strong> esta conexão integra catálogo e pedidos. O redirecionamento do tema é configurado separadamente pelo canal oficial da SOLID.</p></aside></section></main>;
}
