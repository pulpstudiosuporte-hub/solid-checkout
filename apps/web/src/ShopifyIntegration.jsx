...
function ShopifyIntegrationDetails({ csrfToken, storeKey, onBack }) {
  ...
  useEffect(() => { const value = new URLSearchParams(window.location.hash.split('?')[1] || '').get('shopify'); if (value === 'connected') setMessage('Shopify conectada com sucesso. Agora sincronize o catálogo.'); else if (value === 'already_connected') setMessage('Esta loja Shopify já está ativa em outra loja Pirat. Desconecte-a da loja anterior antes de transferir.'); else if (value) setMessage('Não foi possível concluir a conexão. Tente novamente.'); if (value) window.history.replaceState({}, '', '/#/integrations'); }, []);
  ...
  const tutorialAction = action => {
    ...
    if (action === 'dashboard') {
      window.open('https://dev.shopify.com/dashboard', '_blank', 'noopener,noreferrer');
      return;
    }
    const target = document.getElementById(action === 'sync' ? 'shopify-sync-button' : 'shopify-connection-panel');
    target?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    if (action === 'credentials') window.setTimeout(() => document.getElementById('shopify-client-id')?.focus(), 350);
  };
