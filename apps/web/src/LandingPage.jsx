...
const faqs = [
  ['O que é a SOLID Checkout?', 'É uma plataforma para criar e gerenciar checkouts com pagamento Pix. Você reúne a experiência de compra, a personalização da marca, as ofertas e o acompanhamento dos pedidos em uma operação conectada.'],
  ['Preciso usar Shopify?', 'Não. A SOLID tem catálogo e checkouts próprios. Se sua operação usa Shopify, você também pode conectar a loja para integrar o catálogo e a sincronização dos pedidos.'],
  ...
  ['Como funcionam os planos e as tarifas?', 'As opções e condições vigentes ficam disponíveis na área de planos do painel. Consulte os limites e as tarifas da SOLID e do gateway antes de ativar sua operação.'],
...
function Brand({ footer = false }) {
  return <a href="#inicio" className={`solid-site-brand ${footer ? 'footer-brand' : ''}`} aria-label="SOLID Checkout, início"><img src="/brand/solid-wordmark-light.png" width="152" height="51" alt="SOLID"/><span>checkout</span></a>;
}
...
function CheckoutDemo({ compact = false }) {
  ...
  return <div className={`site-demo ${color} ${compact ? 'compact' : ''}`}>
    ...
    <div className="demo-customize"><span><SlidersHorizontal size={13}/> Deixe com a sua cara</span><div role="group" aria-label="Cor da demonstração">{[['violet', 'Violeta'], ['blue', 'Azul'], ['green', 'Verde']].map(([value, label]) => <button key={value} className={`color-swatch ${value}`} aria-label={label} aria-pressed={color === value} onClick={() => setColor(value)}>{color === value && <Check size={12}/>}</button>)}</div></div>
  </div>;
}

function FeatureVisual({ active }) {
  if (active === 'marca') return <div className="feature-visual brand-visual"><div className="mini-editor"><div><Palette size={17}/><strong>Identidade da loja</strong></div><span>Paleta da marca</span><div className="editor-swatches"><i/><i/><i/><i/></div><span>Seu domínio</span><p><LockKeyhole size={13}/> checkout.suamarca.com <CircleCheck size={15}/></p><div className="editor-save"><Check size={13}/> Uma experiência com a sua identidade</div></div><div className="brand-orbit"><img src="/brand/solid-symbol-96.png" alt="" width="64" height="64"/></div></div>;
...
export default function LandingPage() {
  ...
  useEffect(() => {
    // Load the optional font after rendering so a slow provider cannot hold the lazy route open.
    ...
    document.title = 'SOLID Checkout — Sua marca. Seu checkout. Sua próxima venda.';
    document.body.classList.add('solid-marketing-body');
    const escape = event => { if (event.key === 'Escape') setMenuOpen(false); };
    window.addEventListener('keydown', escape);
    return () => { font.remove(); document.title = previous; document.body.classList.remove('solid-marketing-body'); window.removeEventListener('keydown', escape); };
  }, []);
  return <div className="solid-site" id="inicio">
    <a className="site-skip" href="#conteudo">Pular para o conteúdo</a>
    <header className="site-header"><div className="site-container header-inner"><Brand/><nav aria-label="Navegação principal" className={menuOpen ? 'site-nav open' : 'site-nav'} id="site-navigation">{[['Recursos', '#recursos'], ['Como funciona', '#como-funciona'], ['Integrações', '#integracoes'], ['Dúvidas', '#duvidas']].map(([label, href]) => <a href={href} key={href} onClick={() => setMenuOpen(false)}>{label}</a>)}</nav><div className="header-actions"><a href={login} className="site-login">Entrar <ArrowUpRight size={14}/></a><a href={signup} className="site-button small">Começar agora <ArrowRight size={14}/></a><button className="site-menu-toggle" aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} aria-expanded={menuOpen} aria-controls="site-navigation" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={22}/> : <Menu size={22}/>}</button></div></div></header>
    <main id="conteudo">
      <section className="site-hero site-container" aria-labelledby="hero-title">
        <div className="hero-copy"><span className="site-eyebrow"><i/> SUA MARCA. SUA PRÓXIMA VENDA.</span><h1 id="hero-title">Grandes marcas<br/>merecem um<br/><span>checkout à altura.</span></h1><p>A experiência que conecta sua marca à próxima venda. Checkout Pix, personalização e gestão em um só lugar.</p><div className="hero-actions"><a href={signup} className="site-button">Criar minha conta <ArrowUpRight size={19}/></a><a href="#demonstracao" className="site-text-link"><span className="play-icon"><MousePointer2 size={15}/></span> Conhecer na prática</a></div><div className="hero-footnote"><span><Check size={14}/> Sua identidade</span><span><Check size={14}/> Seu domínio</span><span><Check size={14}/> Seu controle</span></div></div>
        <div className="hero-stage" id="demonstracao"><div className="hero-grid" aria-hidden="true"/><div className="stage-orbit orbit-one" aria-hidden="true"/><div className="stage-orbit orbit-two" aria-hidden="true"/><div className="demo-floating-label"><span><Zap size={15}/></span><div><b>O próximo passo da sua loja.</b><small>Do carrinho ao pagamento.</small></div></div><CheckoutDemo/><div className="demo-floating-bottom"><div><Check size={14}/></div><span>Uma jornada.<br/><strong>Todo o controle.</strong></span><svg width="85" height="33" viewBox="0 0 85 33" aria-hidden="true"><path d="M2 29 15 23 24 26 39 13 49 18 62 8 72 12 83 2" fill="none" stroke="#ac97ff" strokeWidth="2.5"/></svg></div><span className="try-hint"><MousePointer2 size={15}/> Pode clicar. O checkout é interativo.</span></div>
      </section>
