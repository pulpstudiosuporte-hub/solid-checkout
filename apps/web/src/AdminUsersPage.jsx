...
export default function AdminUsersPage({ csrfToken }) {
  const [status, setStatus] = useState('PENDING'); const [data, setData] = useState(null); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(''); const [error, setError] = useState(''); const [benefitUser, setBenefitUser] = useState(null);
  ...
  return <main className="page admin-users-page">
    <section className="page-title"><div><p className="eyebrow">ADMINISTRAÇÃO</p><h1>Usuários da plataforma</h1><p>Aprove contas, bloqueie acessos e conceda benefícios comerciais.</p></div><button className="secondary" onClick={load} disabled={loading}><RefreshCw size={17}/> Atualizar</button></section>
    {error && <div className="admin-users-error" role="alert">{error}</div>}
