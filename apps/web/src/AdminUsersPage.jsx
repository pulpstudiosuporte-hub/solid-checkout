...
import { SupportStartModal } from './SupportAccess';
...
export default function AdminUsersPage({ csrfToken, operator }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { setData(await getAdminUsers(status, page, search)); } catch (requestError) { setError(requestError.message); } finally { setLoading(false); } }, [status, page, search]);
  ...
  return <main className="page admin-users-page">
    ...
    {error && <div className="admin-users-error" role="alert">{error}</div>}
    <form className="platform-search" onSubmit={event => { event.preventDefault(); setPage(1); setSearch(query); }}><input aria-label="Buscar usuários" placeholder="Nome ou e-mail" value={query} maxLength={120} onChange={event => setQuery(event.target.value)}/><button className="secondary">Buscar</button></form><section className="card admin-users-card"><div className="admin-users-tabs">{tabs.map(([value, label]) => <button key={value} className={status === value ? 'active' : ''} onClick={() => { setStatus(value); setPage(1); }}>{label}</button>)}</div>
      {loading ? <div className="admin-users-state"><LoaderCircle className="spin"/><span>Carregando usuários...</span></div> : !data?.users?.length ? <div className="admin-users-state"><Users/><b>Nenhum usuário nesta categoria</b><span>Os novos cadastros aparecerão aqui após confirmar o e-mail.</span></div> : <div className="table-wrap"><table className="admin-users-table"><thead><tr><th>Usuário</th><th>Loja</th><th>Plano e taxa</th><th>Cadastro</th><th>Status</th><th>Ações</th></tr></thead><tbody>{data.users.map(user => <tr key={user.publicId}><td><div className="admin-user"><span>{user.name.split(' ').slice(0,2).map(part => part[0]).join('').toUpperCase()}</span><div><b>{user.name}</b><small>{user.email}</small></div>{user.platformAdmin && <em><ShieldCheck size={13}/> Admin Pirat</em>}</div></td><td>{user.memberships?.map(member => member.store.name).join(', ') || 'Sem loja'}</td><td><div className="admin-billing-cell"><b>{planLabel[user.billing?.plan] ?? 'Start'} · {((user.billing?.feeBasisPoints ?? 200) / 100).toLocaleString('pt-BR')}%</b>{user.billing?.sponsored && <span>Patrocinado{user.billing.expiresAt ? ` até ${new Date(user.billing.expiresAt).toLocaleDateString('pt-BR')}` : ''}</span>}</div></td><td>{new Date(user.createdAt).toLocaleDateString('pt-BR')}</td><td><span className={`admin-status ${user.accountStatus.toLowerCase()}`}>{statusLabel[user.accountStatus]}</span></td><td><div className="admin-user-actions">{canPlatform(operator, 'support.read') && !user.platformAdmin && !user.platformRole && user.accountStatus === 'APPROVED' && <button className="benefit" onClick={() => setSupportUser(user)}><ShieldCheck size={15}/> Acessar suporte</button>}{!user.platformAdmin && !user.platformRole && canPlatform(operator, 'billing.manage') && <button className="benefit" onClick={() => setBenefitUser(user)}><BadgePercent size={15}/> Benefício</button>}{canPlatform(operator, 'users.manage') && !user.platformAdmin && !user.platformRole && user.accountStatus === 'REJECTED' && <button className="approve" disabled={busy === user.publicId} onClick={() => approve(user)}><Check size={15}/> Reativar</button>}{canPlatform(operator, 'users.manage') && !user.platformRole && user.accountStatus !== 'REJECTED' && !user.platformAdmin && <button className="block" disabled={busy === user.publicId} onClick={() => block(user)}><UserX size={15}/> Bloquear</button>}</div></td></tr>)}</tbody></table></div>}
    </section><nav className="platform-pagination" aria-label="Páginas de usuários"><button className="secondary" disabled={loading || page === 1} onClick={() => setPage(value => value - 1)}>Anterior</button><span>Página {page} de {data?.pagination?.pages || 1}</span><button className="secondary" disabled={loading || page >= (data?.pagination?.pages || 1)} onClick={() => setPage(value => value + 1)}>Próxima</button></nav>{supportUser && <SupportStartModal user={supportUser} operator={operator} csrfToken={csrfToken} onClose={() => setSupportUser(null)}/>}{benefitUser && <BillingOverrideModal user={benefitUser} csrfToken={csrfToken} onClose={() => setBenefitUser(null)} onSaved={load}/>}
  </main>;
}
