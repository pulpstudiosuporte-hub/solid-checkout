...
import { canPlatform, canPlatformPage, platformPagePermission, supportPages } from './platform-access';
...
function Sidebar({ open, collapsed, onClose, onToggleCollapsed, page, setPage, user, onLogout, stores, storeBusy, onSelectStore, onCreateStore, onArchiveStore, support }) {
  ...
  return <>
    ...
    <aside ref={drawer} aria-label="Navegação da loja" className={`sidebar ${open ? 'open' : ''} ${collapsed ? 'collapsed' : ''}`}>
      ...
      <StoreSwitcher stores={stores} busy={storeBusy} onSelect={onSelectStore} onCreate={onCreateStore} onArchive={onArchiveStore} support={Boolean(support)}/>
      <nav aria-label="Menu principal">
        <section className="sidebar-navigation">
          ...
          <div className="resource-panel" id="sidebar-resources" role="tabpanel">
            ...
            <div className="resource-list">{activeGroup.items.filter(item => !support || supportPages.includes(item.label)).map(item => <button aria-current={page === item.label ? "page" : undefined} key={item.label} title={collapsed ? item.label : undefined} className={page === item.label ? 'nav-item active' : 'nav-item'} onClick={() => navigate(item.label)}><item.icon size={18} aria-hidden="true"/><span>{item.label}</span>{item.count && <em>{item.count}</em>}</button>)}</div>
          </div>
        </section>
      ...
      <div className="side-bottom">
        ...
        {!support && <button title={collapsed ? 'Meu plano' : undefined} className={page === 'Meu plano' ? 'nav-item active' : 'nav-item'} onClick={() => navigate('Meu plano')}><CreditCard size={18}/><span>Meu plano</span></button>}
