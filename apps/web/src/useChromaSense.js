...
const describeTarget = (rawTarget) => {
  ...
  const explicitLabel = element.getAttribute("data-chroma-label");
  const labelByTag = { BUTTON: "Botão", A: "Link", INPUT: "Campo", SELECT: "Seleção", TEXTAREA: "Campo de texto", LABEL: "Rótulo" };
  return {
    target: targetName,
...
export function useChromaSense(sessionId, token) {
  useEffect(() => {
    ...
    const recentClicks = [];
    const payload = (events) => ({ events, viewport: { width: window.innerWidth, height: window.innerHeight }, device: deviceType(), pageKey: "checkout", visitId });
