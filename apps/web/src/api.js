...
export async function getOrders(filters = {}, signal) { const query = new URLSearchParams({ page: String(filters.page || 1), pageSize: String(filters.pageSize || 20) }); return readJson(await fetch(`${apiBaseUrl}/orders?${query}`, { credentials: 'include', headers: { Accept: 'application/json' }, signal })); }
...
export async function getAbandonedCarts(filters = {}, signal) { const query = new URLSearchParams({ page: String(filters.page || 1), pageSize: String(filters.pageSize || 20) }); if (filters.status) query.set('status', filters.status); return readJson(await fetch(`${apiBaseUrl}/abandoned-carts?${query}`, { credentials: 'include', headers: { Accept: 'application/json' }, signal })); }
