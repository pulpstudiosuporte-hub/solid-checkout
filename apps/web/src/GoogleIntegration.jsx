      ...
      <form className="card google-form" onSubmit={save}>
        ...
        <fieldset disabled={busy || !state.writable}>
          ...
          <div className="google-modes">
            <label className={form.mode === 'direct' ? 'selected' : ''}><input type="radio" name="google-mode" value="direct" checked={form.mode === 'direct'} onChange={() => changeMode('direct')}/><span><strong>GA4 e Google Ads</strong><small>Configuração direta pela Pirat</small></span></label>
          ...
          </> : <><div className="google-field"><label htmlFor="google-container">ID do contêiner</label><input id="google-container" value={form.containerId} onChange={event => update('containerId', event.target.value)} placeholder="GTM-XXXXXXX" maxLength={24} required autoComplete="off" spellCheck={false}/><small>Use um contêiner Web do Google Tag Manager.</small></div><p className="google-note">Nesse modo, publique as tags de GA4 e Ads no Tag Manager. A Pirat disponibiliza os eventos no dataLayer. Configure o consentimento e use somente tags necessárias ao checkout.</p></>}
