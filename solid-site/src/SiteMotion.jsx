...
export function MotionScene({ children }) {
  ...
  useEffect(() => {
    ...
    scene.querySelectorAll('.section-heading, .feature-panel, .foundation-card, .setup-guide, .integration-detail, .faq-list, .final-cta, .motion-dashboard').forEach((element, index) => {
    ...
    scene.querySelectorAll('.hero-stage, .integration-network, .final-cta, .feature-sandbox').forEach(element => ambience.observe(element));
    return () => {
...
export function SuccessBurst() {
  return <div className="success-burst" aria-hidden="true">{Array.from({ length: 18 }, (_, index) => <i key={index} style={{ '--burst-x': `${Math.cos(index * 2.4) * (65 + index * 4)}px`, '--burst-y': `${Math.sin(index * 2.4) * 90 - 70}px`, '--burst-angle': `${index * 47}deg`, '--burst-delay': `${index % 4 * 45}ms`, '--burst-color': ['#a17bea', '#68c9ab', '#ecd187'][index % 3] }}/>)}</div>;
}
