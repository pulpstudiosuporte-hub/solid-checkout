...
export default function GlowHorizonFM({ className, variant = "top" }: GlowHorizonProps) {
  ...
  return (
    <motion.div
      aria-hidden="true"
      className={className}
      style={{ position: "absolute", width: "100%", height: "100%", isolation: "isolate", pointerEvents: "none" }}
...
function Arc({ variant, color, size, initialOffset, blur, boxShadow, delay }: {
  ...
  return (
    <motion.div
      aria-hidden="true"
      style={{
        position: "absolute", inset: 0, borderRadius: "100%", scale, background: color,
        ...(blur !== undefined && { filter: `blur(${blur}px)` }),
        ...(boxShadow && { boxShadow }),
      }}
      initial={!reducedMotion && startPct ? { [axis]: startPct } : false}
