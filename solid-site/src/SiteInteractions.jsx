...
export function CheckoutDemo() {
  ...
  const [customer, setCustomer] = useState({ name: 'Alex', email: 'alex@example.com' });
  ...
  const titles = ['Monte seu pedido', 'Quem recebe o pedido?', 'Escolha a entrega', 'Revise e experimente o Pix', 'Explore o pagamento Pix', 'Jornada concluída'];
  const go = next => {
    setStep(next);
    setCopyMessage('');
  ...
  const reset = () => {
    ...
    setCustomer({ name: 'Alex', email: 'alex@example.com' }); go(0);
  };
  ...
  const copy = async () => {
    try {
      await navigator.clipboard.writeText('PIRAT-SEM-VALOR-DE-PAGAMENTO');
      setCopyMessage('Texto copiado. Sem valor para pagamento.');
    } catch {
      setCopyMessage('Não foi possível copiar. Selecione o texto abaixo.');
    }
  };
  return <div className={`site-demo interactive-demo ${color}`}>
    ...
    <div className="demo-content">
      <div className="demo-brand"><span>essentials<span className="accent-dot">.</span></span><span>ESSENTIALS STORE</span></div>
      <nav className="demo-step-nav" aria-label="Etapas do checkout">{['Pedido', 'Dados', 'Entrega', 'Pix'].map((label, index) => <button key={label} aria-current={Math.min(step, 3) === index ? 'step' : undefined} disabled={index > step || step >= 4} onClick={() => go(index)}><span>{index < step ? <Check size={12}/> : index + 1}</span>{label}</button>)}</nav>
      <h2 key={step} className="demo-step-title" ref={heading} tabIndex={-1}>{titles[step]}</h2>
      {step === 0 && <>
        <p className="demo-help">Altere o pedido e veja o total mudar.</p>
      ...
      {step === 1 && <form id={`${id}-customer`} className="demo-form" onSubmit={event => { event.preventDefault(); go(2); }}>
        <p className="demo-help">Explore os campos. Nada é enviado ou salvo ao sair da página.</p>
        <label>Nome<input aria-label="Nome" required maxLength={60} value={customer.name} onChange={e => setCustomer({ ...customer, name: e.target.value })} pattern=".*\S.*" autoComplete="off"/></label>
        <label>E-mail<input aria-label="E-mail" required type="email" maxLength={100} value={customer.email} onChange={e => setCustomer({ ...customer, email: e.target.value })} autoComplete="off"/></label>
      </form>}
      {step === 2 && <fieldset className="demo-delivery"><legend>Opções de entrega</legend><p className="demo-help">Escolha uma opção para ver o frete no resumo do pedido.</p>{deliveryOptions.map(option => <label key={option.id}><input aria-label={`${option.name}, ${option.time}, ${option.price ? money(option.price) : "Grátis"}`} type="radio" name={`${id}-shipping`} checked={delivery === option.id} onChange={() => setDelivery(option.id)}/><span><strong>{option.name}</strong><small>{option.time}</small></span><b>{option.price ? money(option.price) : 'Grátis'}</b></label>)}</fieldset>}
      {step === 3 && <>
        ...
        <div className="demo-review"><span><strong>{shipping.name}</strong><small>{shipping.time}</small></span><button onClick={() => go(2)}>Editar entrega</button></div>
        <div className="demo-method"><div><QrCode size={23}/><span><strong>Pagamento Pix</strong><small>Explore a etapa de pagamento.</small></span></div><Check size={16}/></div>
      </>}
      {step < 4 && <>
      ...
      {step === 4 && <div className="demo-pix"><span className="demo-qr-placeholder"><QrCode size={64}/></span><b>{money(total)}</b><p className="demo-help">Este símbolo representa o QR Code. Não é um código de pagamento. Em uma compra real, o cliente paga no banco e aguarda a confirmação.</p><button className="demo-secondary" onClick={copy}><Copy size={15}/> Copiar texto</button><code>PIRAT-SEM-VALOR-DE-PAGAMENTO</code><p className="demo-help" role="status">{copyMessage || 'Continue para conhecer a confirmação.'}</p><button className="demo-pay" onClick={() => go(5)}>Ver confirmação <Check size={16}/></button><button className="demo-link" onClick={() => go(3)}>Voltar e revisar</button></div>}
      {step === 5 && <div className="demo-success" role="status"><SuccessBurst/><span className="demo-success-icon"><CheckCheck size={31}/></span><p>Experiência concluída.</p><span>Você percorreu um pedido de {money(total)}. Nenhuma cobrança ou e-mail foi enviado.</span><button onClick={() => go(0)}><RotateCcw size={13}/> Experimentar de novo</button><button onClick={reset}>Limpar escolhas e recomeçar</button></div>}
      <div className="demo-caption"><ShieldCheck size={12}/> Ambiente interativo · Sem cobranças.</div>
    </div>
    <div className="demo-customize"><span><SlidersHorizontal size={13}/> Experimente outra cor</span><div role="group" aria-label="Cor do checkout">{colors.map(([value, label]) => <button key={value} className={`color-swatch ${value}`} aria-label={label} aria-pressed={color === value} onClick={() => setColor(value)}>{color === value && <Check size={12}/>}</button>)}</div></div>
  </div>;
}
...
const guideSteps = [
  ...
  { title: 'Publique e acompanhe', subtitle: 'Revisão e lançamento', text: 'Antes de divulgar, percorra uma compra no seu checkout e confira cada etapa.', checks: ['Revisar dados, entrega e total em desktop e celular', 'Validar o fluxo de pagamento no ambiente adequado', 'Compartilhar o link e acompanhar os pedidos'] },
];
